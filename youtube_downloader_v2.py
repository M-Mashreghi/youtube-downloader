...
# https://www.youtube.com/watch?v=grZigSpVDno
# Download a video
video_url_480 = ""
video_url_720 = ""
video_url_1080 = ""
...
# https://youtube.com/playlist?list=PLZHQObOWTQDPD3MizzM2xVFitgF8hE_ab&si=f0HCWGpezhe8rpHW
# Download a playlist :
...
import os
import zipfile
def create_multiple_zip_files(folder_name='download', max_size=1024 * 1024 * 1024, destination_folder='/content'):
    # Define the folder name and the maximum size of each zip file in bytes
    ...
    # Initialize variables for the zip file name and the total size
    zip_name = folder_name + "_1.zip"
    zip_size = 0
    ...
    for file in files:
      ...
      # Check if adding the file will exceed the maximum size
      if zip_size + file_size > max_size:
        # Close the current zip file
        zip_file.close()

        # Increment the zip file name
        zip_name = zip_name[:-5] + str(int(zip_name[-5]) + 1) + ".zip"

        # Reset the zip file size
        zip_size = 0
